...
<Note>
This guide makes references to several configuration settings. These settings will be referenced relative to the the config class that contains them, e.g. `OptimizerConfig.lr`. Please see [API reference](/reference/api) for more details on configuration settings.
</Note>
...
4. CUDA graph to eliminate repeated static host code execution
   ...
   > 3. See [CUDA Graphs](/training/cuda-graphs) for configuration details (`cuda_graph_impl`, `cuda_graph_scope`).
...
- `TransformerConfig.cuda_graph_impl` / `cuda_graph_scope` (see [CUDA Graphs](/training/cuda-graphs))
