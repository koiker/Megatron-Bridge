...
-> See the [Post-Training Quantization section](/model-optimization/quantization#post-training-quantization-ptq) for complete PTQ workflows (quantize, resume and generate, export)

**🏋️ Train with quantization**
-> Check the [Quantization-Aware Training section](/model-optimization/quantization#quantization-aware-training-qat) for QAT workflows

**🎓 Distill a teacher model into a smaller student**
-> See [Knowledge Distillation](/model-optimization/distillation) for the KD configuration and recipe workflow

**✂️ Prune a model to reduce size**
-> See [Pruning](/model-optimization/pruning) for Minitron-based pruning to a target parameter count or fixed architecture
